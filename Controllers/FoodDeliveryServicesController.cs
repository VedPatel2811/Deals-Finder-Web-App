...
namespace Lab5.Controllers
{
    public class FoodDeliveryServicesController : Controller
    {
        ...
        {
            var query = _context.FoodDeliveryServices
                .Include(f => f.Subscriptions)
                    .ThenInclude(s => s.Customer) // Load Customers subscribed to the service
                .AsNoTracking()
                .OrderBy(f => f.Title);

            if (!string.IsNullOrEmpty(ID))
            {
                query = (IOrderedQueryable<FoodDeliveryService>)query.Where(f => f.Id == ID);
            }

            var viewModel = new DealsViewModel
            {
                FoodDeliveryServices = await query.ToListAsync()
            };
